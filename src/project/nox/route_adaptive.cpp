...
/* ROUTE_ADAPTIVE.C - Implements the Fully Adaptive routing algorithm */
...
void adaptive_route(int cn, int dn, int pc, int vc, int *dest_pc, int *dest_vc)
/* returns the pc to go out */
{
  ...
  else if(topology == TORUS)
  {
    ...
    ydist = (dy > cy)? ((dy-cy <= hy)? dy-cy : NUM_ROWS-(dy-cy)) : ((cy-dy< hy)? cy-dy : NUM_ROWS-(cy-dy) );
  }// if TORUS
  ...
  // If this flit is in the deterministic routing VC (In escape path), switch to the deterministic routing.
  ...
  //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  // This flit is not in the deterministic routing VC. 
  if(xdist == 0)
  { // destination is in vertical direction.
    cur_dir = ydir;
    /*
  ...
  { // destination is in horizontal direction.
    cur_dir = xdir;
    /*
    if(credit[xdir] > 0)        cur_dir = xdir;
    else if(credit[ydir] > 0)   cur_dir = ydir;// xdist == 0 && dir == ydir. Cannot use VC0. 
    else                         cur_dir = xdir;
    */
  }
  else
  { // destination is in diagonal direction.
    if(xdist > ydist)
    {
      if(credit[xdir] > 0)      cur_dir = xdir;
  ...
  *dest_pc = cur_dir; 
  *dest_vc = (best_vc[cur_dir] != -1)? best_vc[cur_dir] : vc;
