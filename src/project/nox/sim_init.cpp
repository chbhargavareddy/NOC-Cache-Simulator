...
void sim_init(int argc, char **argv)
{
  ...
  cmsg_total_flit_latency       = 0;
  cmsg_total_msg_latency        = 0;
  cmsg_total_queueing_latency   = 0;
  cmsg_total_network_latency    = 0;

  dmsg_total_delay_cycle        = 0;
  dmsg_total_flit_latency       = 0;
  dmsg_total_msg_latency        = 0;
  ...
  while(count < argc)
  {
    ...
    else if(!strcmp(argv[count], "-r"))
    {
	  ...
	  else if(!strcmp(argv[count], "OE")) routing_algorithm = OE;
	  else if(!strcmp(argv[count], "NL")) routing_algorithm = NL;
    ...
    else if(!strcmp(argv[count], "-t"))
    {
      ...
      else if(!strcmp(argv[count], "FBFLY"))  topology = FBFLY;
      else if(!strcmp(argv[count], "MECS"))  topology = MECS;
    ...
    else if(!strcmp(argv[count], "-p"))
    {
      ...
      else if(!strcmp(argv[count], "CUST")) pattern = CUST;
  ...
  // Flatterned butterfly
  if(topology == FBFLY || topology == MECS)
  {
    NUM_NODES = NUM_ROWS * NUM_COLS; 
    concentration_degree = (NUM_PE / NUM_NODES >= 1)?
                           (NUM_PE / NUM_NODES):1; 
    
    if(concentration_degree != 4 && concentration_degree != 1) 
    { 
      printf("(!) Only support 1- and 4-degree of concentration now, "
             "input concentration degree %d\n", concentration_degree);
      //exit(1); 
    }  
    if(concentration_degree > 1) 
      concentrated = YES; 
 
    NUM_PC      = concentration_degree + (NUM_ROWS - 1) + (NUM_COLS - 1); 
    NUM_NIC_PC  = concentration_degree;  
    link2       = (NUM_ROWS > NUM_COLS)?NUM_ROWS - 1:NUM_COLS - 1; 
    for(int node = 0;node < NUM_NODES;node ++) 
    {
      router_info[node].num_pc     = NUM_PC; 
      router_info[node].num_nic_pc = NUM_NIC_PC; 
    ...
    // For FBFLY, initialize credit tables 
    for(int node = 0;node < NUM_NODES;node ++) 
...
//#endif 
  ...
  {
    //Default memory controller layout
    //FIXME: only works for even number of MCs
    //for(int i = 0;i < num_mem_controllers/2;i ++)
    //{
    //  mem_network_layout[i] = NUM_NODES/2 - i;
    //  mem_network_layout[num_mem_controllers - i - 1] 
    //                        = NUM_NODES/2 + i + 1; 
    //} 
